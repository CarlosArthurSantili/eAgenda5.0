...
namespace eAgenda.Controladores.CompromissoModule
{
    public class ControladorCompromisso : Controlador<Compromisso>
    {
        ...
        AND ((@HORA_INICIO_DESEJADO >= HORAINICIO AND @HORA_INICIO_DESEJADO < HORATERMINO) 
            OR (@HORA_TERMINO_DESEJADO > HORAINICIO AND @HORA_TERMINO_DESEJADO <= HORATERMINO)
            OR (@HORA_INICIO_DESEJADO <= HORAINICIO AND @HORA_TERMINO_DESEJADO >= HORATERMINO))";
        #endregion
        ...
        public override string InserirNovo(Compromisso registro)
        {
            ...
            {
                bool horarioOcupado = !ValidarConflitos(registro.Data, registro.HoraInicio, registro.HoraTermino);
                if (horarioOcupado)
        ...
        private bool ValidarConflitos(DateTime data, TimeSpan horaInicio, TimeSpan horaTermino)
        {
            Dictionary<string, object> DicionarioParametros = new Dictionary<string, object>()
            {
                {"DATA", data}, 
                {"HORA_INICIO_DESEJADO", horaInicio.Ticks},
                {"HORA_TERMINO_DESEJADO", horaTermino.Ticks}
            };
